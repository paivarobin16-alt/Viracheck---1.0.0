    ...
    <div style={{ minHeight: "100vh", background: "#0b0d12", padding: 18 }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", color: "#fff", fontFamily: "system-ui" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}>
          <section style={card}>
            <h2 style={h2}>1) Upload do vídeo</h2>
          ...
          <section style={card}>
            ...
            <label style={label}>Gancho</label>
            ...
            <button onClick={handleAnalyze} disabled={loading} style={{ ...button, opacity: loading ? 0.7 : 1 }}>
            ...
            {result && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div style={scoreBox}>
                  ...
                  {result.cached && <div style={{ marginTop: 6, opacity: 0.85 }}>✅ Resultado do cache</div>}
                </div>

                <ListBlock title="✅ Pontos fortes" items={result.strengths} />
                <ListBlock title="⚠️ Pontos fracos" items={result.weaknesses} />
                <ListBlock title="🔧 Melhorias" items={result.improvements} />

                <TextBlock title="🧠 Título" text={result.title} />
                <TextBlock title="✍️ Legenda" text={result.caption} />
                <TextBlock title="📣 CTA" text={result.cta} />

                {result.frame_insights?.length ? <ListBlock title="🖼️ Insights dos frames" items={result.frame_insights} /> : null}
              </div>
            )}
          </section>
        </div>
...
const h2: any = { margin: "0 0 12px", fontSize: 16, color: "rgba(255,255,255,0.9)" };
const label: any = { display: "block", marginTop: 10, marginBottom: 6, opacity: 0.8, fontSize: 13 };
...
const progressBox: any = { marginTop: 10, padding: 10, borderRadius: 12, background: "rgba(255,255,255,0.06)", fontSize: 13 };
const errorBox: any = { marginTop: 10, padding: 10, borderRadius: 12, background: "rgba(255,0,0,0.12)", fontSize: 13 };
...
const block: any = { padding: 14, borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" };
const h3: any = { margin: "0 0 8px", fontSize: 14 };
import { useMemo, useState } from "react";
import { analyzeVideo, VideoAnalysisResult } from "../services/videoAnalysisService";
import { extractFramesFromVideoFile } from "../utils/frameExtractor";
import { makeVideoFingerprint } from "../utils/fingerprint";
